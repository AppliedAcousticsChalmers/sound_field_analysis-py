...
def test_sph_harm():
    ...
    assert sph.sph_harm(m=0, n=0, az=0, co=0, kind="complex") == approx(
        0.282094791773878
    )
    ...
    assert sph.sph_harm(m=0, n=1, az=0.1, co=0.1, kind="complex") == approx(
        0.486161534508712
    )

    # AKsh(2, -2, rad2deg(-0.1), rad2deg(-0.1), 'complex')
    assert sph.sph_harm(m=-2, n=2, az=-0.1, co=-0.1, kind="complex") == approx(
    ...
    assert sph.sph_harm(m=17, n=17, az=7, co=6, kind="complex") == approx(
        -2.202722537106273e-10 + 8.811259608538045e-11j
    )

    # AKsh(17, -17, rad2deg(6), rad2deg(-7), 'complex')
    assert sph.sph_harm(m=-17, n=17, az=6, co=-7, kind="complex") == approx(
        4.945682459644794e-05 - 4.843297071701655e-04j
    )

    # AKsh(0, 0, 0, 0, 'real')
    assert sph.sph_harm(m=0, n=0, az=0, co=0, kind="real") == approx(0.282094791773878)

    # AKsh(1, 0, rad2deg(0.1), rad2deg(0.1), 'real')
    assert sph.sph_harm(m=0, n=1, az=0.1, co=0.1, kind="real") == approx(
        0.486161534508712
    )

    # AKsh(2, -2, rad2deg(-0.1), rad2deg(-0.1), 'real')
    assert sph.sph_harm(m=-2, n=2, az=-0.1, co=-0.1, kind="real") == approx(
        0.001081666550095
    )
    ...
    assert sph.sph_harm(m=17, n=17, az=7, co=6, kind="real") == approx(
        3.115120086120565e-10
    )

    # AKsh(17, -17, rad2deg(6), rad2deg(-7), 'real')
    assert sph.sph_harm(m=-17, n=17, az=6, co=-7, kind="real") == approx(
